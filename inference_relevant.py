...
def run_inference(model, processor, data, args, device):
    ...
    results = []
    for query, temporal_window, rel in zip(querys, temporal_windows, relevant):
        ...
        relevance, predictions = extract_relevant(output_text)
        is_relevant = relevance[0]
        predictions = predictions.numpy()
        if retrieval_mode =='mr':
            if not rel:
                print("gt :not relevant")
            if not is_relevant:
                print(output_text)
            data_dict = {
                "qid": qid,
            ...
            # Long video - mr_seg mode
            if not is_relevant:
                # If not relevant in mr_seg, directly record result without refinement
                data_dict = {
                    "qid": qid,
                    "id": vid,
                    "annos": [
                    ...
                    "pred_relevant_windows_mr_seg": [[-1]]
                }
                results.append(data_dict)
            else:
                pred_relevant_windows_mr_seg = data.get("pred_relevant_windows_mr_seg", None)
